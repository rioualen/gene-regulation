...
2. Choose appliance "Ubuntu 14.04 IFB-X2GO-10GB" in the drop-down menu. TODO Change
3. Name your VM.
...
mkdir -p ~/mydisk/GSE20870_analysis/source/GSE20870/GSM521934 
wget -v --no-clobber ftp://ftp-trace.ncbi.nlm.nih.gov/sra/sra-instant/reads/ByExp/sra/SRX%2FSRX021%2FSRX021358/SRR051929/SRR051929.sra -P ~/mydisk/GSE20870_analysis/source/GSE20870/GSM521934/

## Download ChIP-seq reads
mkdir -p ~/mydisk/GSE20870_analysis/source/GSE20870/GSM521935
wget -v --no-clobber ftp://ftp-trace.ncbi.nlm.nih.gov/sra/sra-instant/reads/ByExp/sra/SRX%2FSRX021%2FSRX021359/SRR051930/SRR051930.sra -P ~/mydisk/GSE20870_analysis/source/GSE20870/GSM521935/
```


<!--
For convenience, we create a symlink from our preferred location `/data` to the data directory `~/mydisk/GSE20870_analysis`. 

```
ln -s ~/mydisk/GSE20870_analysis ~/data
```
-->

Note: the commands above can be adapted to download the data in any other directory. You then need to update the workflow configuration file `gene-regulation/examples/GSE20870/GSE20870.yml`. 

## 1.6 Run the workflow

```
cd  ~/mydisk/GSE20870_analysis/
ln -s ~/gene-regulation-1.0 ./gene-regulation
...
You can visualize the peaks by running IGV from the terminal. You may need to source the `~/.bashrc` first in order to update teh `$PATH`. 
```
source ~/.bashrc
igv
```
