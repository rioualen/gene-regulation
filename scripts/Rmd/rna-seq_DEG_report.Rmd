...
#required.libraries <- append(required.libraries, "xlsx") ## THIS REQUIRES JAVA
...
count.boxplot(all.counts, sample.desc, 
              xlab="Raw counts", 
              main="Raw counts per sample")

## Boxplot of log10-transformed counts
count.boxplot(all.counts.log10, sample.desc,  xlab="log10(counts)", 
              main="log10(counts) per sample")

## Boxplot of CPMs
count.boxplot(cpms, sample.desc, xlab="CPM", 
              main="CPMs per sample")

## Boxplot of log10-transformed CPMs
count.boxplot(cpms.log10, sample.desc, xlab="log10(CPM)", 
...
```{r read_design_table, warning=FALSE}
# setwd(dir.main) ## !!!!! I don't understand why I have to reset the working directory at each chunk
...
design.table <- read.delim(file.path(dir.base, design.file), sep="\t", 
...
i <- 1
comparison.summary <- design.table ## Initialize a summary table for each DEG analysis
comparison.summary$prefixes <- paste(sep="_", design.table[,1], "vs", design.table[,2])

for (i in 1:nrow(design.table)) {
  ## Identify samples for the first condition
  cond1 <- as.vector(design.table[i,1])  ## First condition for the current comparison
  ...
  cond2 <- as.vector(design.table[i,2])  ## Second condition for the current comparison
  ...
  verbose(paste(sep="", "\tDifferential analysis\t", i , "/", nrow(design.table), "\t", cond1, " vs ", cond2), 1)
