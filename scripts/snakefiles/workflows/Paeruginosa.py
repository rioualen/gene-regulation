...
MAPPED_READS_BWA = expand(RESULTS_DIR + "{samples}/{samples}_{aligner}.sam", samples=SAMPLE_IDS, aligner=config["bwa"]["suffix"])
#if verbosity >= 3:
#    print("\nMAPPED_READS_BWA\n\t" + "\n\t".join(MAPPED_READS_BWA))

# Sorted and converted reads (bam, bed)
SORTED_MAPPED_READS_BWA = expand(RESULTS_DIR + "{sample}/{sample}_{aligner}_sorted_pos.bam", sample=SAMPLE_IDS, aligner=config["bwa"]["suffix"])
BAM_READNB = expand(RESULTS_DIR + "{samples}/{samples}_{aligner}_sorted_pos_bam_readnb.txt", samples=SAMPLE_IDS, aligner=config["bwa"]["suffix"])
SORTED_READS_BED = expand(RESULTS_DIR + "{sample}/{sample}_{aligner}_sorted_pos.bed", sample=SAMPLE_IDS, aligner=config["bwa"]["suffix"])
BED_READNB = expand(RESULTS_DIR + "{samples}/{samples}_{aligner}_sorted_pos_bed_nb.txt", samples=SAMPLE_IDS, aligner=config["bwa"]["suffix"])
#if verbosity >= 3: 
